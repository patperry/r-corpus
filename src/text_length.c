...
#define BAIL(msg) \
	do { \
		while (ngroup-- > 0) { \
			corpus_intset_destroy(&types[ngroup]); \
		} \
		Rf_error(msg); \
	} while (0)
...
SEXP text_ntype(SEXP sx, SEXP sfilter, SEXP sweights, SEXP sgroup)
{
	...
	types = (void *)R_alloc(ngroup, sizeof(*types));
	for (g = 0; g < ngroup; g++) {
		if ((err = corpus_intset_init(&types[g]))) {
			while (g-- > 0) {
				corpus_intset_destroy(&types[g]);
			}
	...
	for (i = 0; i < n; i++) {
		...
		while (corpus_filter_advance(filter)) {
			...
			if ((err = corpus_intset_add(&types[g],
						     filter->type_id,
						     NULL))) {
				BAIL("memory allocation failure");
			}
