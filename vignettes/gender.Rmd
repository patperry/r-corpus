...
![plot of chunk unnamed-chunk-14](gender-unnamed-chunk-14-1.png)
...
```
Error: <text>:2:1: unexpected symbol
1: log2_he_se <- abs((1 / (log(2) * he)) * he_se
2: log2_she_se
   ^
```
...
```
Error in eval(expr, envir, enclos): object 'log2_she_se' not found
```
...
```
Error in eval(expr, envir, enclos): object 'log2_ratio_se' not found
```
...
```
Error in xy.coords(x, y, xlabel, ylabel, log): object 'ylim' not found
```
...
```
Error in r[i]: invalid subscript type 'special'
```
...
```
Error in (r - 0.4)[i]: invalid subscript type 'special'
```
...
```
Error in print(log2_ratio_se[["stopped"]]): object 'log2_ratio_se' not found
```
...
```
Error in print(log2_ratio_se[["felt"]]): object 'log2_ratio_se' not found
```
...
```
Error in print(log2_ratio_se[["was"]]): object 'log2_ratio_se' not found
```
...
```
Error in est[signif]: invalid subscript type 'special'
```
...
```
Error in segments(est - se, i, est + se, i, lwd = 1.5, col = col): object 'se' not found
```
...
```
Error in segments(est - se, i - 0.2, est - se, i + 0.2, lwd = 1.5, col = col): object 'se' not found
```
...
![plot of chunk unnamed-chunk-24](gender-unnamed-chunk-24-1.png)
