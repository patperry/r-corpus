...
title <- trimws(sub("^[[:space:]]*[[:digit:]]+[.][[:space:]]*", "", lines[chapter]))
```
...
tokens, use the `text_length` function. To count the number of non-dropped
tokens, use the `text_ntoken` function.
...
tokens; `text_ntoken` counts only non-dropped tokens.


Here are the last 10 tokens in each chapter:


```r
last <- text_sub(data, -10)
print(last, -1)
```
...
ch21 as Dorothy was safely on her\nway to Kansas.               
ch22 and the others\nwere to be admitted at once.               
ch23 air, and were\nlost forever in the desert.                 
ch24 I'm so glad to be at home again!"                          
```

Note that if we do not specify the ending position, it defaults to `-1`.
...
!['witch' Occurences](corpus-witch-occurences-1.png)
...
19 ch10 …st journey on until we find the road of   yellow   brick again," said\nDorothy, "and then …
...
19 ch08 …hall be carried into the country of the   Wicked   Witch of the West,\nand she will enchan…
20 ch11 … silver shoes?"\n\n"I got them from the   Wicked   Witch of the East, when my house fell o…
⋮  (72 rows total)
```
...
19 ch06 …art, I have no heart; so I cannot\nhave   heart    disease."\n\n"Perhaps," said the Lion t…
